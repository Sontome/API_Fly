...
def login(p, username=USERNAME, password=PASSWORD):
    ...
    page.goto("https://www.accounts.amadeus.com/LoginService/authorizeAngular?service=ARD_9G-DC&client_id=1ASIXARD9GDC&LANGUAGE=GB&redirect_uri=https%3A%2F%2Ftc110.resdesktop.altea.amadeus.com%2Fapp_ard%2Fapf%2Finit%2Flogin%3FSITE%3DA9GPAIDL%26LANGUAGE%3DGB%26MARKETS%3DARDW_PROD_WBP%26ACTION%3DclpLogin#/login")
    ...
    if jsession_data:
        with open("session_log_sun.json", "w", encoding="utf-8") as f:
            json.dump(jsession_data, f, indent=2, ensure_ascii=False)

        cookies = page.context.cookies()
        with open("cookie1a_sun.json", "w", encoding="utf-8") as f:
            ...
            try:
                while True:
                    if not unlock_queue.empty():
                        ...
                        if msg == "check_unlock":
                            # Check 1: Alert box thông báo (bấm OK)
                            try:
                                ...
                                print("[⚠] Phát hiện alert box — đang bấm OK...")
                                page.click("#uicAlertBox_ok")
                                print("[✅] Đã đóng alert box!")
                            except TimeoutError:
                                pass

                            # Check 2: Password lock screen
                            try:
                                page.wait_for_selector(
