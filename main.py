...
@app.post("/process-pdf-vna-en/")
async def process_pdf_VNA_EN(
    background_tasks: BackgroundTasks,
    file: UploadFile = File(...),
    option: str = Form("")
):
    # Tạo đường dẫn file tạm input
    temp_path = os.path.join(TEMP_DIR, f"EN_{file.filename}")
    ...
    # Tạo đường dẫn file output
    output_path = os.path.join(TEMP_DIR, f"output_EN_{file.filename}")
