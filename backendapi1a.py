...
async def giu_ve_live_cmd(hanhkhach, dep, arr, depdate, deptime, arrdate=None, arrtime=None, doituong="VFR"):
    ...
    try:
        async with httpx.AsyncClient(http2=False, timeout=60) as client:
            ...
            for cmd in ["APE-hanvietair.service@gmail.com","APE-hanvietair.service@gmail.com/p1","APM-+82 1035463396","APM-+82 1035463396/p1","APN-E+HANVIETAIR.SERVICE@GMAIL.COM/VI/p1","APN-M+821035463396/VI/P1","TK OK", "RF HVA", "ER", "IG"]:
                print(f"✈️ Gửi {cmd}")
                ssid, res = await send_command(client, cmd, "giuvelive")
                responses.append({
                    "cmd": cmd,
